...
const DashboardPage = async () => {
  ...
  const totalCourses = Array.isArray(stats?.courses)
    ? stats.courses.length
    : (stats?.courses ?? 0);
  const totalEnrollments = Array.isArray(stats?.enrollments)
  ...
  if (Array.isArray(stats?.enrollments) && stats.enrollments.length > 0) {
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="shadow-sm transition-shadow hover:shadow-md">
          ...
          <CardContent>
            <div className="text-3xl font-bold">{avgRating}/5.0</div>
            <p className="mt-1 text-xs text-muted-foreground">
